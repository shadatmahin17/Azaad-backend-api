...
## Supabase setup
1. Copy `.env.example` to `.env` (or `.env.local`) and add your Supabase values.
2. Install server dependencies including Supabase SDK: `npm install @supabase/supabase-js dotenv`.
3. If you also have a Next.js frontend, install SSR helpers there: `npm install @supabase/ssr`.
4. Run `supabase/schema.sql` in Supabase SQL editor to create the `profiles` table and policies.
5. In Supabase Auth, enable Email/Password provider.
6. Send the returned `accessToken` from `/api/auth/signin` as `Authorization: Bearer <token>` when calling profile endpoints.
7. You can also sign in via `POST /api/login` using `{ "email": "...", "password": "..." }` (or put the email in `username` for legacy clients).
