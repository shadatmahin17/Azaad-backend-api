...
function ensureSupabaseReady(res) {
  if (!supabaseAdmin) {
    res.status(500).json({
      error: 'Supabase is not configured. Install @supabase/supabase-js and set NEXT_PUBLIC_SUPABASE_URL + NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY (or SUPABASE_SERVICE_ROLE_KEY).'
    });
    return false;
  }
  return true;
